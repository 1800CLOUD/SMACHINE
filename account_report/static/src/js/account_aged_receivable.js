odoo.define('account_report.account_aged_receivable',  function (require) {
'use strict';

var core = require('web.core');
var datepicker = require('web.datepicker');
var accountReportsWidgetExtended = require('account_reports.account_report');
...
accountReportsWidgetExtended.include({
    ...
    custom_events: {
        'value_changed': function(ev) {
            ...
            self.report_options.vendor_ids = ev.data.vendor_ids;
            self.report_options.sale_team_ids = ev.data.sale_team_ids;
            ...
            return self.reload().then(function () {
                ...
                self.$searchview_buttons.find('.account_sales_filter').click();
    ...
    render_searchview_buttons: function() {
        ...
        this.M2MFilters = false;
        // Sales filter
        if (this.report_options.sales) {
            if (!this.M2MFilters) {
                var fields = {};
                if ('vendor_ids' in this.report_options) {
                    fields['vendor_ids'] = {
                        label: _t('Salesman'),
                        modelName: 'res.users',
                        value: this.report_options.vendor_ids.map(Number),
                    }
                };
                if ('sale_team_ids' in this.report_options) {
                    fields['sale_team_ids'] = {
                        label: _t('Sale Team'),
                        modelName: 'crm.team',
                        value: this.report_options.sale_team_ids.map(Number),
                    }
                };
                if (!_.isEmpty(fields)) {
                    this.M2MFilters = new M2MFilters(this, fields);
                    this.M2MFilters.appendTo(this.$searchview_buttons.find('.js_account_sales_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_sales_m2m').append(this.M2MFilters.$el);
            }
        }
        this.M2MFilters = false;
