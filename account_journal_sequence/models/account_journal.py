...
class AccountJournal(models.Model):
    ...
    sequence_id = fields.Many2one(
        ...
        "numbering of the journal entries of this journal.",
        copy=False
    )
    ...
    def _create_sequence(self, vals, refund=False):
        ...
        seq_date_range.number_next = refund and \
    ...
    @api.depends(
        'refund_sequence_id.use_date_range',
    ...
    def _inverse_refund_seq_number_next(self):
        '''
        Inverse 'refund_sequence_number_next'
         to edit the current sequence next number.
        '''
        for journal in self:
            if journal.refund_sequence_id and \
                    journal.refund_sequence and \
                    journal.refund_sequence_number_next:
                sequence = journal.refund_sequence_id._get_current_sequence()
    ...
    def create(self, vals):
        ...
        if vals.get('type') in ('sale', 'purchase') and \
                vals.get('refund_sequence') and \
                not vals.get('refund_sequence_id'):
            vals.update({'refund_sequence_id': self.sudo(
            )._create_sequence(vals, refund=True).id})
        journal = super(AccountJournal, self.with_context(
