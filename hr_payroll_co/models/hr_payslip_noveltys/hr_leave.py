...
class HrLeave(models.Model):
    ...
    def _prepare_leave_line(self, periods):
        ...
        while date_tmp <= self.date_end:
            if not(date_tmp.day == 31 and type_id.category_type != 'VAC_MONEY' and not type_id.apply_day_31):
                ...
                if type_id.category_type == 'SICKNESS' and politics.get('hr_payroll_co.pays_eg_b2_with_wage', False) and sequence <= 2:
                    amount_real = economic_variables['wage'] / 30
                elif type_id.category_type == 'AT_EP' and politics.get('hr_payroll_co.pays_atep_b1_with_wage', False) and sequence == 1:
                    amount_real = economic_variables['wage'] / 30
                else:
                    amount_real = amount * \
    ...
    def _VAC_MONEY(self, economic_variables):
        if self.days_vac_money <= 0:
            raise ValidationError(
                f'La cantidad de días {self.days_vac_money}, debe ser mayor a 0')
    ...
    def _alter_holiday_book(self):
        holiday_book_line = {
            'contract_id': self.contract_id.id,
            'leave_id': self.id,
        }
        if self.category_type == 'VAC_MONEY':
            holiday_book_line['days_paid'] = self.days_vac_money
        elif self.category_type == 'VAC':
            holiday_book_line['days_enjoyed'] = len(self.leave_line_ids)
        else:
            holiday_book_line = {}

        if holiday_book_line:
            self.env['hr.holiday.book'].create(holiday_book_line)

    def _clean_holiday_book(self):
